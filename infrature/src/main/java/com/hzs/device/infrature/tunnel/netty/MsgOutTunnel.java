...
public class MsgOutTunnel {
    ...
    public static boolean sentToClient(final Channel channel, byte[] msg) {
        ...
        try {
            ...
            log.info("MsgOutTunnel sentToClient, ctx:{} buf:{}", channel.isActive(), buf);
            if (!channel.isActive()){
                log.warn("MsgOutTunnel sentToClient, channel if not active.");
                return false;
            }
    ...
    public void sentToClient(final ChannelHandlerContext ctx, byte[] msg) {
        ...
        final ByteBuf buf = Unpooled.copiedBuffer(msg);
        log.info("MsgOutTunnel sentToClient, ctx:{} buf:{}", ctx.channel().isActive(), buf);
        executor.execute(new Runnable() {
            @Override
            public void run() {
